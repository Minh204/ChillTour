...
public class ContractsController : Controller
{
    ...
    [HttpGet("/contracts/{id:long}/preview")]
    public async Task<IActionResult> Preview(long id, CancellationToken cancellationToken)
    {
        ...
        var bytes = _contractService.GeneratePdf(contract);
        Response.Headers.ContentDisposition = $"inline; filename=\"{contract.ContractCode}.pdf\"";
        return File(bytes, "application/pdf");
    }

    private async Task<ElectronicContract?> LoadAuthorizedContractAsync(long id, long userId, CancellationToken cancellationToken)
    {
    ...
    private ContractSignViewModel BuildViewModel(ElectronicContract contract)
    {
    ...
    private bool CanUserSign(ElectronicContract contract, long userId)
    {
        ...
        return contract.ContractStatus == ContractService.PendingCustomerSign && contract.Booking.UserId == userId;
    }
